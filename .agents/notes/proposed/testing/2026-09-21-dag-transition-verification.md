...
- `model-property.spec.ts` under `packages/dag/dag/tests/` runs under `pnpm run test`, generates command sequences without precondition filtering, and asserts all four properties of the proposal; a failing run reproduces from a logged seed.
- `crash-recovery.spec.ts` under `packages/dag/dag/tests/` passes with a real `JsonlSessionPersistence` root, asserts at every reopen that the `dag/state` fold satisfies `validateDagState` at each step, and asserts that `reconcile` leaves no non-settled mailbox command.
