...
`dag/state` is declared by module augmentation and joins the known session-event vocabulary; it does not carry `ignorable: true`, so a build that does not know the event refuses the log rather than misreading the snapshot. The event has one reader, `latestState` (`packages/dag/dag/src/index.ts:1379-1386`), which scans the session's snapshots backwards for the last `dag/state`; there is no process-local mirror. `DagService.mutate` (`index.ts:730-757`) reads that snapshot, calls `reduceDagState` (`packages/dag/dag/src/reducer.ts:432`), returns unchanged when the reducer returns the same object (`index.ts:733`), and otherwise appends the new snapshot (`index.ts:737`) inside one synchronous block with no `await`.
...
Every external effect runs after its command is durable: `pump` appends `command-running` and flushes (`packages/dag/dag/src/index.ts:819`), `prepareAndStart` flushes after `git-prepared` (`index.ts:977`), `ensureWave` flushes after `wave-probed` (`index.ts:1025`), and `deliverOwnerRedirect` flushes before admitting the replacement (`packages/dag/dag/src/index.ts:627`). `ctx.sessions.flush` returns whether at least one durability listener participated (`packages/core/session/src/index.ts:1205-1235`), and it is a real barrier because the agent loop's stored session holds a JSONL write handle; that is a property of the agent loop, not of `flush`.
...
`DagGit.prepare` captures `currentHead` at `packages/dag/dag/src/git.ts:135`, before the dependency merge loop at `:157`, and returns it as `preparedFrom` at `:178`. `prepareAndStart` re-runs `prepare` — never `verifyPrepared` — whenever the durable node has no `preparedHead` (`packages/dag/dag/src/index.ts:941`), and the `git-prepared` append that records the result happens later (`index.ts:959`, flush at `:977`).
...
7. **Migrate the DAG's synchronous Session reads (one to two days).** Seven production calls — `latestState`, `dispatcherFor`, the child turn boundary, `messageRecorded`, `noticeRecorded`, the invariant's install-time seed, and `SubagentContinuationManager.recordsMessage` — read event history directly and carry the repository's deferred-migration waiver. The maintained state they need is exactly what the client-visible `dag` projection withholds: the complete `DagState`, the recorded message and notice identities, the child descriptor, and the current turn boundary. Migrating them is therefore a durable-state and projection design change with its own format question, not a mechanical replacement.
