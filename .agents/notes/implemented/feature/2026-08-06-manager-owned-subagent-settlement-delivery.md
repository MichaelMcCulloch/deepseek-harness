...
**Add a deployment-wide opt-out or let the model choose delivery.** Rejected. Ordinary delegation needs the `adaptive` default so its model-facing promise stays unconditional. An explicit child creator can select `quiet` or `none` only when it owns the alternate delivery path; the model cannot choose the policy.
...
- Unit coverage pins `adaptive`, `quiet`, and `none`, owner terminal delegation, each terminal reason, idle and busy scheduling, the batch, the maintenance regression, the pre-release ordering, a parent that is gone, and a rejected send that must not fail teardown.
