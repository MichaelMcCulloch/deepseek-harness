...
`ctx.subagents.interrupt(targetSessionId, authority)` 只停止在线目标的当前轮次。管理器原语同步完成鉴权并返回 `void`，不等待目标完全停稳。对于普通 child，它调用 `Agent.cancel(cause, { keepInbox: true })`。对于 owner-bound child，它把已鉴权请求和一次性 stop closure 委派给已注册 owner controller；controller 可以先提交 owner 状态，再调用同一取消操作。通用生命周期的其余部分不变：不 dispose Activation、不释放 handle、不级联后代、不清空 inbox。由于 `keepInbox` 让尚未领取的待处理队列停在 idle，中断绝不会自动启动下一个排队的 follow-up；已被领取进入中断轮次的工作属于该轮次，不会重新入队。被中断的 driver 进入 idle 后，一次显式唤醒发送会按保留的 FIFO 顺序恢复。[原生 DAG owner](../architecture/2026-08-29-event-sourced-native-dag-orchestrator.zh.md)使用这项委派在取消前提交 `interrupted`。
...
人类或 ancestor 可以停止一个失控的 continuable 轮次，而不丢失 child、其尚未领取的排队工作或正在运行的后代；代价是一个刻意保持弱的通用后置条件（`accepted` 表示“信号已发出”，目标在观察到信号前可能仍显示 `running`），客户端必须如实呈现。owner controller 可以在调用取消前增加更强的状态后置条件。暂停队列规则意味着被中断的 child 会带着保留的工作停在 idle，直到 driver 进入 idle 后收到唤醒消息。
...
`dsh-tool-subagent-control` 中面向模型的 `interrupt_agent(agent_id)` 工具把 `exec.agent` 作为 `ancestor` 授权传入，自身不增加任何权限：核心原语会在任何 owner 委派前校验在线注册表身份与记录的 lineage，因此该工具可以用同一个通用 `agent_id` 参数指定直接 child 或更深的后代——刻意不用会暗示仅限直接 child 的 `subagent_id`。发现依赖 `list_agents({ scope: 'descendants' })`，其底层是 `SubagentRuntime.listDescendants()`，每个条目带经校验的 `parentId` 与 `depth`（列表约定由[持久化目录 note](2026-07-22-durable-subagent-catalog-and-list-agents.zh.md)拥有）；发现只是提示，绝非权限。`send_message` 与 `steer_agent` 保持确切直接 parent 权限——只有 interrupt 是 ancestor 级的。

## 测试

`packages/subagent/subagent/tests/continuation.spec.ts` 中的核心覆盖证明持久化 `turn/end` 中止、队列先暂停后按 FIFO 恢复、后代不受影响、两种授权及其取消 cause、鉴权后的 owner 委派、self/sibling/stale/非 ancestor 拒绝、absent/one-shot/disposal 竞态 no-op，以及 `keepInbox` 循环行为不变。`packages/api/gateway/tests/` 中的 Host 覆盖证明 RPC 只调用核心原语、`subagent-unauthorized` 与 `internal` 映射、wire schema 的 continuable 模式限制，以及 carrier 往返。Client 与组装 Web 覆盖固定按地址路由的 Stop 操作、parent 离线行为、Send 入队、follow-up 暂停和 FIFO 恢复。工具覆盖证明直接与更深 ancestor interrupt、鉴权失败不触碰目标、目标不存在时 no-op 且不冷恢复，以及 descendants 列表；已录制的 request header 固定这些 schema。
