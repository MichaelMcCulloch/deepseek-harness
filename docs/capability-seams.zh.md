...
flowchart LR
  ...
  pkg_dag["dag"]
  svc_dag["ctx.dag<br/>Durable DAG orchestration domain"]
...
| `ctx.dag` | `core` | [`dag`](../packages/dag/dag) | - | [`tool-dag`](../packages/dag/tool-dag) | - | 在调度受 generation 防护的 Git、子 agent、通知和生命周期 effect 之前，提交完整且带修订版本的图快照。 |
