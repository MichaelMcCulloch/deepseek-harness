...
Preparation is durable in two phases through one `git-prepared` command. The first records the worktree's pre-merge HEAD as both `preparedFrom` and `preparedHead` before any dependency merge runs; the second advances only `preparedHead` past the merges. A crash between the merges and their record therefore cannot change what `preparedFrom` means: the retry resumes the merge phase, which is a no-op per already-merged commit, instead of re-observing a worktree whose HEAD is now the merge commit. A node whose merge is already recorded ignores any further phase.
...
Reset is available only for pending or failed nodes. It accepts the frozen wave base, an exact commit id, or an explicit local `refs/heads/*` ref. It rejects remote refs and ambiguous names. It aborts an active merge, hard-resets tracked state, keeps untracked files, and reports remaining dirt. It also repairs a worktree directory that exists without a Git registration, which is what an interrupted `git worktree add` leaves behind and what every later preparation refuses: reset prunes the stale registration, removes the residue, and adds the worktree again. The removal is bounded to the DAG's own worktree root, so a path the dispatcher did not create is refused rather than deleted. The service does not delete old branches, worktrees, or child sessions.
