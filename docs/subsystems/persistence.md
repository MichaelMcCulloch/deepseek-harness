...
`dag/state` is one known typed member of that event vocabulary. Its domain payload contains a complete version-1 scheduler snapshot, but adding the member does not change the JSONL header or event envelope. `SESSION_FORMAT_VERSION` therefore stays `0`; a build that does not know `dag/state` refuses it through the event-vocabulary check instead of misreading or skipping it. The [DAG subsystem](dag.md) owns the payload version and replay rules.
