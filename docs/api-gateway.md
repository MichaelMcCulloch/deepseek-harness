...
The subagent control namespace shows why two similar unary methods remain distinct. `subagent.prompt` admits a later FIFO turn, while `subagent.steer` interrupts active child work and admits its replacement before queued ordinary turns. Both return the accepted inbox message id; the Remote response does not claim that the child turn completed. `subagent.interruptByParent` and both delivery methods keep authorization and owner delegation in the Host service rather than exposing owner metadata on the wire.
