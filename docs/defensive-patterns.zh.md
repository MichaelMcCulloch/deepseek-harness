...
这些规则防止已知的生命周期、并发、子进程与清理缺陷。真实入口路径、观测结果与资源归属的测试规则见 [testing.md](testing.zh.md)。
...
在外部工作前提交状态。运行持久 mailbox 时不得跨 `await` 持有锁；按 generation 与 operation 为回调设 fence，并在 cancellation 前记录 stop。[原生 DAG 调度器](subsystems/dag.zh.md)应用此规则。

<a id="dispose-must-reach-quiescence-not-just-request-it"></a>

## dispose 必须达到完全停稳，而不仅仅是请求停止

如果清理流程在被终止或中止的工作停止前返回，就会留下孤儿进程。应等待子级退出，并在终止前关闭监听器与通知注册表，使迟到的完成事件保持静默。
