...
| `@deepseek-ai/dsh-tool-dag` | `dag_dispatch`、`dag_node_block`、`dag_node_complete`、`dag_node_inspect`、`dag_node_redispatch`、`dag_node_reset`、`dag_node_resume`、`dag_node_steer`、`dag_node_stop`、`dag_status`、`dag_wait`、`dag_write` | `ctx.tools`、`ctx.dag`、`ctx.systemPrompt`、`an owning dispatcher or DAG child Agent` | `tool/call`、`dag/state for mutations`、`tool/result` | - | 调度器获得十个图控制工具。owner-bound DAG 子级只获得 dag_status、dag_node_complete 与 dag_node_block。本目录合并两组作用域 schema；实际子级组合会移除全部调度器控制。 |
...
| `@deepseek-ai/dsh-tool-subagent-control` | `interrupt_agent`、`list_agents`、`send_message`、`steer_agent` | `ctx.tools`、`ctx.subagents`、`ctx.agents and ctx.sessionProjections (list_agents only)` | `tool/call`、`tool/result`、`child session events through ctx.subagents` | - | 这些是控制可续行后台 subagent 的全局命名工具：绑定 provider 的 `tool-subagent` 实例注册不同的委派工具；本包注册一次 `send_message`、`steer_agent` 与 `interrupt_agent`，另由单独加载的 `/list-agents` 插件提供 `list_agents`。`send_message` 添加较后的 FIFO 轮次；`steer_agent` 取消并替换当前工作。 |
...
启动互不重复且依赖已就绪的 pending 节点。响应不等待 Git 或子级创建完成。
...
来源：[`packages/dag/tool-dag/src/index.ts`](../packages/dag/tool-dag/src/index.ts)

### `dag_node_block`

工作无法继续时，用明确原因暂停本 DAG 节点。
...
报告已提交且干净的工作，以执行本地 Git 验证。本调用只接受命令；只有验证成功后节点才会完成。
...
把一个 failed 节点重新设为 pending，供后续 dag_dispatch 调用使用。
...
把 tracked worktree 状态 reset 到冻结 base、精确 commit 或本地 refs/heads ref。untracked 文件会保留。
...
用新指令恢复一个 blocked 或 interrupted 节点。
...
中断并替换活跃节点工作。已暂停节点会通过 starting 返回。
...
读取当前 DAG 面板。工作运行时使用 dag_wait，不要重复调用 status。
...
等待较后的可操作 DAG 通知被注入。dispatch 后使用本工具，不要轮询状态。
...
声明或修改完整依赖图。每次调用都发送全部节点。既有节点必须重复其不可变字段与精确 live status。
...
调度器获得十个图控制工具。owner-bound DAG 子级只获得 dag_status、dag_node_complete 与 dag_node_block。本目录合并两组作用域 schema；实际子级组合会移除全部调度器控制。
...
中断后台 subagent 的当前轮次，并用新指令替换它。待处理 inbox 消息继续排队，但该替换轮次先于已排队的普通轮次运行。子级保留其对话与持久会话。本调用只返回被接受的 message id，不等待子级回答。失败表示替换内容**未**投递。
...
这些是控制可续行后台 subagent 的全局命名工具：绑定 provider 的 `tool-subagent` 实例注册不同的委派工具；本包注册一次 `send_message`、`steer_agent` 与 `interrupt_agent`，另由单独加载的 `/list-agents` 插件提供 `list_agents`。`send_message` 添加较后的 FIFO 轮次；`steer_agent` 取消并替换当前工作。
