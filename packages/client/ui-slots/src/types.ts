 ...
 * registration records, the explicit scope-target map, and the standard Session
 * area seat. Types only; the entry module re-exports every declaration here, so
 * `@deepseek-ai/dsh-client-ui-slots` stays the single public API.
 * @module @deepseek-ai/dsh-client-ui-slots/src/types
 */
/* oxlint-disable typescript/no-redundant-type-constituents --
 * `keyof SlotScopeTargetMap & 'session'` is the declare-merge key pattern:
 * SlotScopeTargetMap is empty in THIS compilation unit (so the intersection
 * reads as `never`), but consumer `declare module` augmentation merges keys in
 * and the intersection is what keeps them narrowed to the `session` scope. The
 * rule fires on the empty-map view, not on real redundancy. */
