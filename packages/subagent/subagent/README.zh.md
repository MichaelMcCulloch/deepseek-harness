...
`dsh-subagent` 是子 agent 委派背后的服务：agent 把任务交给具名子 agent，收集完成结果，并且——对可继续子 agent 而言——跨重启发送较后工作、redirect 活跃工作或 stop 轮次。多个提供方在同一约定下共存，因此单个组合可以并排提供进程内子 agent、进程外 ACP 或 SDK 子 agent，以及真实 Codex 或 Claude Code 子 agent。子 agent 有两种形态：一次性运行以单个结果 settle；可继续子 agent 的持久会话接受较后消息与 owner-scoped 生命周期控制。同一服务还回答发现类问题——存在哪些子级、它们的模式、活动状态与血缘——而不加载或恢复它们。把它与至少一个提供方后端和一个委派工具一起挂载；后端与面向模型的工具位于兄弟包中。
...
一次性子 agent 只运行一次，并以单个结果 settle，可附带可选结构化输出与失败时的安全诊断。启动请求可以通过 `agentOptions` 覆盖子 Agent 的提供方、模型、推理等级与输出 token 上限；每个请求的选项都要求提供方声明对应能力。可继续子 agent 保留持久会话并按顺序接受较后消息：调用方收到稳定子 agent id、发送 FIFO follow-up、redirect 当前工作，或中断当前轮次而不销毁子 agent。创建可以固定绝对 `cwd`、确定性 child 与 message id、持久 owner metadata，以及通用 settlement-delivery policy。工具行的 `backgroundMode` 选择形态（默认 `one-shot`，或在支持的提供方上使用 `continuable`）。

### Follow-up、redirect、中断与发现

可继续子 agent 把 follow-up 消息作为下一个轮次回答。redirect 在保留 inbox 的情况下取消活跃工作，并在已排队普通轮次前放置一个替换轮次。父级也可以随时中断运行中的轮次或列举自己的子级。发现覆盖两种形态：服务列举直接子级与完整后代树——模式、活动状态与血缘——直接读取在线会话状态与可选持久化，不加载任何子 agent。
...
| [`src/continuation.ts`](src/continuation.ts) | 可继续子级：身份预留、Activation 驻留、follow-up、redirect、中断、settlement |
...
管理器预留子 agent identity、解析持久 descriptor、在其已保存绝对 `cwd` 创建（或冷恢复）子 agent、把它安装进 Activation 并提交提示词。较后的 follow-up 经子 agent 自己的 inbox 成为 FIFO 轮次；redirect 则取消并替换活跃工作。没有 Activation 时从持久会话冷恢复。通用 settlement delivery 是 `adaptive`、`quiet` 或 `none`。可选 owner controller 接收已授权 stop 与 redirect 请求及已捕获终态事实；授权仍留在本服务。
...
- **描述符仅进日志**——它是会话事件，不进入模型历史，并跨 compaction 保留；可继续 descriptor 会显式记录解析后的子级提供方、模型、推理等级、settlement delivery 与可选 owner binding，用于冷恢复。Session header 记录其绝对 `cwd`。
...
- **Host-user delivery 仍需要在线 parent**——prompt 与 steer 请求携带持久地址，但授权会在准入前解析确切在线直接 parent。
- **Follow-up 与 redirect 不同**——parent-to-child follow-up 排入较后轮次；只有 redirect 取消并替换当前工作。
