...
At most one notice per settled Activation in the parent's request, sized by the child's final message. `none` adds no generic notice. A child that sends its own message and then receives generic settlement delivery costs the parent both.
