...
// The pipeline's data types live in their own leaf module so this entry, the
// schema DSL, and the PTC transport can all name them without importing one
// another; re-exported here so `@deepseek-ai/dsh-tools` stays the single
// public API for tool producers and UI adapters.
export type {
  PtcDispatchLog,
  ScheduledToolDispatch,
  ScheduledToolPreparation,
...
export class ToolRuntime extends Service {
  ...
  private requireCodeTransport(): ToolDefinition {
    this.ptcTransport ??= createRunCodeTool(this, {
      ...
      scheduler: this[TOOL_RUNTIME_SCHEDULER],
