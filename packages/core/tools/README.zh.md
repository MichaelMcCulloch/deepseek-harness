...
| [`src/pipeline-types.ts`](src/pipeline-types.ts) | `ToolDefinition`、`ToolExecution`、`ToolExecutionResult` 与分阶段调度类型 |
| [`src/types.ts`](src/types.ts) | 持久的 `tool/ptc-dispatch*` 事件载荷及其 `SessionEventMap` 合并声明 |
