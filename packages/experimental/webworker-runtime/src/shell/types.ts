 ...
 * face a program reads and writes, the program signature the command table
 * stores, and the executables the launcher answers in place of an
 * operating-system binary. A browser worker has no processes, so a "program"
 * is a JavaScript function over the VFS and the state below is the whole
 * machine.
