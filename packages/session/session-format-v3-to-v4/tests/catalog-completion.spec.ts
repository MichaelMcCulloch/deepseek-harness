...
describe('V3 parent catalog completion', () => {
  ...
  it('interprets the current V4 descriptor shape written into V3 sessions', () => {
    expect(childCatalogFact({
      childId: 'child', childCreatedAt: 2, descriptorCount: 1,
      descriptor: { version: 4, mode: 'continuable', provider: 'spawn', label: 'current child', settlementDelivery: 'adaptive' },
    })).toEqual({ version: 0, childId: 'child', childCreatedAt: 2, mode: 'continuable', label: 'current child' })
  })
  ...
  it('retains unknown children while validating available descriptor fields', () => {
    ...
    for (const data of [null, {}, { version: 99, mode: 'one-shot', provider: 'spawn' }]) {
